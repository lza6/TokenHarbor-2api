    ...
    macro_rules! m {
        ($surface:expr, $label:expr, $family:expr, $tier:expr, $pi:expr, $po:expr, $free:expr, $inmods:expr, $outmods:expr, $vision:expr, $tools:expr, $ctx:expr, $mo:expr) => {
            v.push(ModelMeta {
                ...
                reasoning: true,
                free_until: if $free {
                    free_until($surface).map(|s| s.to_string())
                } else {
                    None
                },
                context_window: $ctx,
...
pub fn upstream_id(surface: &str, is_free: bool) -> String {
    ...
    for (s, p) in SURFACE_PROVIDER {
        if *s == surface {
            let p = if *p == "th-rudder" {
                "th-rudder".to_string()
            } else {
                p.to_string()
            };
            return if is_free {
                format!("{p}/{surface}:free")
            } else {
                format!("{p}/{surface}")
            };
        }
...
impl ModelRegistry {
    ...
    pub async fn normalize(&self, requested: &str) -> String {
        ...
        if self.has_model(&free_id).await {
            return free_id;
        }
        let paid_id = upstream_id(surface, want_free);
        if self.has_model(&paid_id).await {
            return paid_id;
        }
        r.to_string()
    }
    ...
    pub async fn resolve(&self, requested: &str) -> String {
        ...
        if self.available(&norm).await {
            return norm;
        }
        for m in self.free_ids().await {
            if self.available(&m).await {
                return m;
            }
        }
...
/// 模型能力检索（面板/工具调用判定）
pub fn supports_tools(meta: &ModelMeta) -> bool {
    meta.tools
}
pub fn supports_vision(meta: &ModelMeta) -> bool {
    meta.vision
}
pub fn supports_search(meta: &ModelMeta) -> bool {
    meta.web_search
}
