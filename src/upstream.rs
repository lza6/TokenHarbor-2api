...
impl UpstreamClient {
    pub fn new(base_url: String, cookie: Option<String>, proxy: Option<String>, _timeout: Duration) -> Result<Self> {
        ...
        Ok(Self { base_url, http, cookie, proxy })
    }
    ...
    /// 创建会话（返回 session id）
    pub async fn create_session(&self, model: &str, temporary: bool, cookie: Option<&str>) -> Result<String> {
        let url = format!("{}/api/direct-chat/sessions", self.base_url);
        let body = SessionRequest { model: model.to_string(), temporary };
        ...
        if !status.is_success() {
            return Err(anyhow!("创建会话失败 HTTP {status}: {}", truncate(&text, 300)));
        }
    ...
    pub async fn get_session(&self, id: &str, cookie: Option<&str>) -> Result<SessionDetail> {
        ...
        let resp = self.http.get(&url).headers(self.cookie_header(cookie)).send().await.context("读取会话失败")?;
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        if !status.is_success() {
            return Err(anyhow!("读取会话失败 HTTP {status}: {}", truncate(&text, 300)));
        }
        serde_json::from_str(&text).with_context(|| "解析会话详情失败".to_string())
    }

    /// 更新会话（把临时会话落库 / 改系统参数）
    pub async fn patch_session(&self, id: &str, body: serde_json::Value, cookie: Option<&str>) -> Result<()> {
        let url = format!("{}/api/direct-chat/sessions/{id}", self.base_url);
        ...
        if !status.is_success() {
            let text = resp.text().await.unwrap_or_default();
            return Err(anyhow!("更新会话失败 HTTP {status}: {}", truncate(&text, 300)));
        }
        Ok(())
    }

    /// 对话流：返回响应体（SSE），由上层逐行转换
    pub async fn stream(&self, req: &StreamRequest, cookie: Option<&str>) -> Result<reqwest::Response> {
    ...
    pub async fn prepare_upload(&self, kind: &str, name: &str, mime: &str, bytes: u64, cookie: Option<&str>) -> Result<UploadResponse> {
        ...
        let text = resp.text().await.unwrap_or_default();
        if !status.is_success() {
            return Err(anyhow!("上传准备失败 HTTP {status}: {}", truncate(&text, 300)));
        }
        serde_json::from_str(&text).with_context(|| "解析上传响应失败".to_string())
    }

    /// 语音转文字
    pub async fn transcribe(&self, audio: Vec<u8>, mime: String, cookie: Option<&str>) -> Result<TranscribeResponse> {
        ...
        let text = resp.text().await.unwrap_or_default();
        if !status.is_success() {
            return Err(anyhow!("语音转文字失败 HTTP {status}: {}", truncate(&text, 300)));
        }
    ...
    pub async fn free_tier(&self, cookie: Option<&str>) -> Result<serde_json::Value> {
        ...
        let resp = self.http.get(&url).headers(self.cookie_header(cookie)).send().await.context("查询免费额度失败")?;
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        if !status.is_success() {
            return Err(anyhow!("查询免费额度失败 HTTP {status}: {}", truncate(&text, 300)));
        }
    ...
    pub async fn chat_quotas(&self, cookie: Option<&str>) -> Result<serde_json::Value> {
        ...
        let resp = self.http.get(&url).headers(self.cookie_header(cookie)).send().await.context("查询每日限额失败")?;
        let status = resp.status();
        let text = resp.text().await.unwrap_or_default();
        if !status.is_success() {
            return Err(anyhow!("查询每日限额失败 HTTP {status}: {}", truncate(&text, 300)));
        }
...
fn default_headers(base_url: &str) -> HeaderMap {
    ...
    h.insert("origin", HeaderValue::from_str(base_url).unwrap_or(HeaderValue::from_static("https://tokenharbor.ai")));
    h.insert(REFERER, HeaderValue::from_str(&format!("{base_url}/chat")).unwrap_or(HeaderValue::from_static("https://tokenharbor.ai/chat")));
