...
async fn normalize_free_surfaces() {
    ...
    assert_eq!(
        r.normalize("qwen3.8-flash").await,
        "alibaba/qwen3.8-flash:free"
    );
    assert_eq!(
        r.normalize("deepseek-v4.1-flash").await,
        "vercel-ai-gateway/deepseek/deepseek-v4.1-flash:free"
    );
    assert_eq!(
        r.normalize("mimo-v2.6-flash:free").await,
    ...
    // 已带 provider 前缀放行
    assert_eq!(
        r.normalize("anthropic/claude-opus-5.5").await,
...
async fn free_window_expired_unavailable() {
    ...
    let later = DateTime::parse_from_rfc3339("2026-09-30T00:00:00+00:00")
        .unwrap()
        .with_timezone(&Utc);
...
fn upstream_id_mapping() {
    assert_eq!(
        models::upstream_id("qwen3.8-flash", true),
        "alibaba/qwen3.8-flash:free"
    );
    assert_eq!(
        models::upstream_id("deepseek-v4.1-flash", true),
        "vercel-ai-gateway/deepseek/deepseek-v4.1-flash:free"
    );
    assert_eq!(
        models::upstream_id("th-rudder", false),
        "th-rudder/th-rudder"
    );
    assert_eq!(
        models::upstream_id("claude-opus-5.5", false),
        "anthropic/claude-opus-5.5"
    );
...
fn catalog_has_expected_models() {
    ...
    assert!(list
        .iter()
        .any(|m| m.id == "vercel-ai-gateway/deepseek/deepseek-v4.1-flash:free"));
